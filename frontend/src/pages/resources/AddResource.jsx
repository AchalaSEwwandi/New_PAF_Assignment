...
const AddResource = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await resourceService.createResource(processedData);
      navigate('/resources');
    } catch (err) {
      setError(err.response?.data?.message || err.message || 'Failed to create resource. Please check your input.');
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#e8f5e9] to-[#e3f2fd] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-2xl w-full bg-white p-10 rounded-3xl shadow-md relative">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-gray-900 tracking-tight">Add New Resource</h2>
        </div>
